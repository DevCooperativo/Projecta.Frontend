...
export const ProjectCategoryDetail = () => {
    ...
    return <>
        <div className="d-flex justify-content-between align-items-start mb-4"><div><nav><ol className="breadcrumb mb-1 small"><li className="breadcrumb-item"><Link to="/">Início</Link></li><li className="breadcrumb-item"><Link to="/project-categories">Categorias de projetos</Link></li><li className="breadcrumb-item active">{item.name}</li></ol></nav><h4 className="fw-bold mb-0">{item.name}</h4><span className="badge bg-light text-dark border mt-1">{item.area}</span></div><div className="d-flex gap-2"><Link className="btn btn-outline-dark" to={`/project-categories/${item.id}/edit`}>Editar</Link><button className="btn btn-outline-danger" onClick={() => setShowModal(true)}>Excluir</button></div></div>
