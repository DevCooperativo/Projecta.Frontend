...
export const LaboratoryDetail = () => {
    ...
    return <>
        ...
        {canManage && <div className="d-flex gap-2"><Link className="btn btn-outline-dark" to={`/laboratories/${item.id}/edit`} state={{ laboratory: item }}>Editar</Link><button className="btn btn-outline-danger" onClick={() => setShowModal(true)}>Excluir</button></div>}</div>
